...
const json = JSON.stringify({
  ...
  const updateComment = async () => {
    ...
    try {
      let response = await fetch(`${API_BASE_URL}/defectiveActs/updateDefAct/`+id, {
        method: 'PUT',
        headers: {
  ...
  const getStructure = async () => {
    ...
    //getPhoto
    try {
      const response = await fetch(`${API_BASE_URL}/defectiveActs/getPhoto/` + id,
        {method: 'GET',
          headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
...
if (contentType != null){
  ...
  console.log('singlePhoto', singlePhoto)
   ...
   const deletePhoto = async () => {
    setDisabled(true);
    try {
      let response = await fetch(`${API_BASE_URL}/defectiveActs/removePhoto/`+id, {
          method: "DELETE",
          //redirect: "follow",
          headers: {
            'Authorization': `Bearer ${accessToken}`,
  ...
  const deleteNote = async () => {
    setDisabled(true);
  try {
      let response = await fetch(`${API_BASE_URL}/defectiveActs/deleteDefAct/`+id, {
          method: "DELETE",
          headers: {
            'Authorization': `Bearer ${accessToken}`,
        },
        ...
        <View style={[styles.container]}>
          ...
          <View style={{ paddingBottom: BOTTOM_SAFE_AREA + 20 }}>
                ...
                />
                  ...
                  <PermissionGuard required='DEFACT_DELETE'>
                    <CustomButton
                    disabled={disabled}
